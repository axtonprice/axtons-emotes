    ...
    */
    public void resetLangFiles(CommandSender sender) {
        ...
        try {
            ...
            sender.sendMessage(MessageHandler.parseSuccess("command.resetlang.error"));
    ...
    /**
     * Resets the emote configuration file to its default state.
     *
     * @param sender The {@link CommandSender} who initiated the reset command.
     */
    public void resetEmoteConfig(CommandSender sender) {
        // Check permissions
        if (!validatePermissions("resetemotes", sender))
            return;

        try {
            // Save default emote config
            Configuration.saveDefaultConfigFile("emotes.yml", true);

            // Reload emote config after reset
            Configuration.reloadConfig("emotes.yml");

        } catch (Exception e) {

            // Log error and notify sender
            sender.sendMessage(MessageHandler.parseError("command.resetemotes.error"));
            Debugging.logError("Emote configuration reset failed", e);
            return;
        }

        // Notify sender and log
        sender.sendMessage(MessageHandler.parseSuccess("command.resetemotes.success"));
        Debugging.log(classMethod(), sender.getName() + " reset emote configuration to default.");
