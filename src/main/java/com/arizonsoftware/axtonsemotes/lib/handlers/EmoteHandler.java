...
import com.arizonsoftware.axtonsemotes.utils.EmoteCooldown;
...
public class EmoteHandler extends FXHandler {
    ...
    */
    private void performSharedEmote(String[] args, Player player) {
        ...
        Boolean hasSenderToggled = PlayerData.retrievePlayerSetting("toggleemotes", player, Boolean.class);
        ...
        Boolean hasTargetToggledEmotes = PlayerData.retrievePlayerSetting("toggleemotes", target, Boolean.class);
        ...
        // Check emote radius if enabled
        if (Configuration.getBoolean("config.yml", "emote-radius.enabled")) {

            // Fetch config and distance
            double requiredDistance = Configuration.getInt("config.yml", "emote-radius.distance");
            double actualDistance = player.getLocation().distance(target.getLocation());

            // Debug logging
            Debugging.log(EmoteHandler.class.getSimpleName() + "/radiusCheck",
                    "Required distance: " + (int) requiredDistance + ", actual distance: " + (int) actualDistance);

            // Check if player has bypass permission
            if (!player.hasPermission("axtonsemotes.admin.bypassradius")) {
                // Check player distance
                if (actualDistance > requiredDistance) {
                    player.sendMessage(MessageHandler.parseError("error.emote.radius"));
                    return;
                }
            }
        }
        ...
        if (isPlayerInCooldown(player)) {
            int cooldownTime = Configuration.getInt("config.yml", "emote-cooldown.duration-seconds");
            player.sendMessage(MessageHandler.parseErrorReplace("error.emote.cooldown",
                    new String[] { "duration" }, new String[] { String
                            .valueOf(cooldownTime) }));
            return;
        }
    ...
    */
    ...
    private boolean isPlayerInCooldown(Player player) {
        if (EmoteCooldown.isCooldownEnabledConfig()) {
            if (EmoteCooldown.isPlayerInCooldown(player)) {
                return true;
            } else {
                // Apply cooldown to player
                EmoteCooldown.setCooldown(player);
                return false;
            }
        }
        return false;
